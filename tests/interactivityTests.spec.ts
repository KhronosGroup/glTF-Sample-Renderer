...
for (const dir of directories) {
    ...
    for (const asset of testAssets) {
        ...
        createInteractivityTest(testName, file);
    }
}

function createInteractivityTest(name: string, file: Uint8Array<ArrayBuffer>) {
    test(`Testing asset ${name}`, async ({ page }) => {
        await page.goto("");
        ...
        let success = false;
        try {
            success = await page.evaluate(async (file) => {
