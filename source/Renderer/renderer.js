...
class gltfRenderer {
    ...
    init(state) {
        ...
        const samples =
            state.internalMSAA < maxSamples ? state.internalMSAA : maxSamples;
        if (!this.initialized) {
            ...
            this.scatterInternalFormat =
                context.supports_EXT_color_buffer_half_float
                    ? context.RGBA16F
                    : context.RGBA;
            this.scatterType = context.supports_EXT_color_buffer_half_float
                ? context.HALF_FLOAT
                : context.UNSIGNED_BYTE;
            ...
            this.maxVertAttributes = context.getParameter(
    ...
    prepareScene(state, scene) {
        ...
        const drawables = this.nodes
            ...
            .reduce(
                (acc, node) =>
                    acc.concat(
                        state.gltf.meshes[node.mesh].primitives.map(
                            (primitive, index) => {
                                return {
                                    node: node,
                                    primitive: primitive,
        ...
        this.opaqueDrawables = drawables.filter(
            ({ primitive }) =>
                state.gltf.materials[primitive.material].alphaMode !==
                    "BLEND" &&
        ...
        this.scatterDrawables = drawables.filter(
            ({ primitive }) =>
                state.gltf.materials[primitive.material].extensions !==
                    undefined &&
                state.gltf.materials[primitive.material].extensions
                    .KHR_materials_volume_scatter !== undefined &&
                state.gltf.materials[primitive.material].extensions
    ...
    drawScene(state, scene) {
        ...
        if (state.cameraNodeIndex === undefined) {
            ...
            currentCamera.perspective.aspectRatio =
                this.currentWidth / this.currentHeight;
        } else {
            currentCamera =
                state.gltf.cameras[
                    state.gltf.nodes[state.cameraNodeIndex].camera
                ];
        ...
        if (currentCamera.type === "perspective") {
            if (currentCamera.perspective.aspectRatio) {
                if (
                    currentCamera.perspective.aspectRatio > currentAspectRatio
                ) {
                    aspectHeight =
                        (aspectWidth * 1) /
                        currentCamera.perspective.aspectRatio;
                    ...
                    aspectWidth =
                        aspectHeight * currentCamera.perspective.aspectRatio;
                }
            }
        } else {
            const orthoAspect =
        ...
        for (const instance of Object.values(this.opaqueDrawables)) {
            ...
            if (instance.length > 1) {
                ...
                if (
                    instance[0].primitive.glAttributes.length + 4 <=
                    this.maxVertAttributes
                ) {
        ...
        if (this.transmissionDrawables.length > 0) {
            ...
            for (const instance of Object.values(this.opaqueDrawables)) {
                ...
                let sampledTextures = {};
                if (scatterEnabled) {
                    sampledTextures.scatterSampleTexture =
                        this.scatterFrontTexture;
                    sampledTextures.scatterDepthSampleTexture =
                        this.scatterDepthTexture;
                }
                this.drawPrimitive(
                    state,
        ...
        for (const drawable of this.transmissionDrawables.filter(
            (a) => a.depth <= 0
        )) {
            let renderpassConfiguration = {};
            renderpassConfiguration.linearOutput = false;
            ...
            let sampledTextures = {};
            sampledTextures.transmissionSampleTexture =
                this.opaqueRenderTexture;
            if (scatterEnabled) {
                sampledTextures.scatterSampleTexture = this.scatterFrontTexture;
                sampledTextures.scatterDepthSampleTexture =
        ...
        for (const drawable of this.transparentDrawables.filter(
            (a) => a.depth <= 0
        )) {
            let renderpassConfiguration = {};
            renderpassConfiguration.linearOutput = false;
    ...
    drawPrimitive(
        ...
        if (material.textureTransforms.length > 0) {
            for (let i = 0; i < material.textureTransforms.length; i++) {
                if (
                    material.textureTransforms[i] !== undefined &&
                    material.textureTransforms[i].key === "Normal"
                ) {
        ...
        if (primitive.mode < 4) {
            ...
            if (
                ...
                fragDefines = fragDefines.filter(
                    (e) =>
                        e !== "HAS_NORMAL_MAP 1" &&
                        e !== "HAS_CLEARCOAT_NORMAL_MAP 1"
                );
            }
        }
        ...
        if (fragmentHash && vertexHash) {
            this.shader = this.shaderCache.getShaderProgram(
                fragmentHash,
                vertexHash
            );
        }
        ...
        if (renderpassConfiguration.scatter) {
            this.webGl.context.uniform1i(
                this.shader.getUniformLocation("u_MaterialID"),
                renderpassConfiguration.drawID
            );
        }
        ...
        for (const attribute of primitive.glAttributes) {
            ...
            if (
                !this.webGl.enableAttribute(state.gltf, location, gltfAccessor)
            ) {
        ...
        if (instanceOffset !== undefined) {
            const location = this.shader.getAttributeLocation(
                "a_instance_model_matrix"
            );
        ...
        this.shader.updateUniform(
            "u_EmissiveFactor",
            jsToGl(material.emissiveFactor)
        );
        ...
        this.shader.updateUniform(
            "u_BaseColorFactor",
            jsToGl(material.pbrMetallicRoughness?.baseColorFactor)
        );

        this.shader.updateUniform(
            "u_AnisotropyUVSet",
            material.extensions?.KHR_materials_anisotropy?.anisotropyTexture
                ?.texCoord
        );

        const factor =
            material.extensions?.KHR_materials_anisotropy?.anisotropyStrength;
        const rotation =
            material.extensions?.KHR_materials_anisotropy?.anisotropyRotation;
        const anisotropy = vec3.fromValues(
            Math.cos(rotation ?? 0),
            Math.sin(rotation ?? 0),
            factor ?? 0.0
        );
        ...
        this.shader.updateUniform(
            "u_ClearcoatUVSet",
            material.extensions?.KHR_materials_clearcoat?.clearcoatTexture
                ?.texCoord
        );
        this.shader.updateUniform(
        ...
        this.shader.updateUniform(
            ...
            material.extensions?.KHR_materials_clearcoat?.clearcoatNormalTexture
                ?.texCoord
        );
        this.shader.updateUniform(
        ...
        this.shader.updateUniform(
            "u_IridescenceFactor",
            material.extensions?.KHR_materials_iridescence?.iridescenceFactor
        );
        this.shader.updateUniform(
            "u_IridescenceIor",
            material.extensions?.KHR_materials_iridescence?.iridescenceIor
        );
        this.shader.updateUniform(
        ...
        this.shader.updateUniform(
            "u_IridescenceUVSet",
            material.extensions?.KHR_materials_iridescence?.iridescenceTexture
                ?.texCoord
        );
        this.shader.updateUniform(
        ...
        this.shader.updateUniform(
            ...
            material.extensions?.KHR_materials_sheen?.sheenRoughnessFactor
        );
        this.shader.updateUniform(
            "u_SheenColorFactor",
            jsToGl(material.extensions?.KHR_materials_sheen?.sheenColorFactor)
        );
        this.shader.updateUniform(
            "u_SheenRoughnessUVSet",
            material.extensions?.KHR_materials_sheen?.sheenRoughnessTexture
                ?.texCoord
        );
        this.shader.updateUniform(
            "u_SheenColorUVSet",
        ...
        this.shader.updateUniform(
            ...
            jsToGl(
                material.extensions?.KHR_materials_specular?.specularColorFactor
            )
        );
        this.shader.updateUniform(
        ...
        this.shader.updateUniform(
            "u_AttenuationColor",
            jsToGl(material.extensions?.KHR_materials_volume?.attenuationColor)
        );
        this.shader.updateUniform(
            "u_AttenuationDistance",
        ...
        this.shader.updateUniform(
            "u_ThicknessUVSet",
            material.extensions?.KHR_materials_volume?.thicknessTexture
                ?.texCoord
        );
        ...
        this.shader.updateUniform(
            "u_DiffuseTransmissionUVSet",
            material.extensions?.KHR_materials_diffuse_transmission
                ?.diffuseTransmissionTexture?.texCoord
        );
        this.shader.updateUniform(
            "u_DiffuseTransmissionColorUVSet",
            material.extensions?.KHR_materials_diffuse_transmission
                ?.diffuseTransmissionColorTexture?.texCoord
        );
        ...
        this.shader.updateUniform(
            ...
            material.extensions?.KHR_materials_pbrSpecularGlossiness
                ?.specularGlossinessTexture?.texCoord
        );
        this.shader.updateUniform(
            "u_DiffuseUVSet",
            material.extensions?.KHR_materials_pbrSpecularGlossiness
                ?.diffuseTexture?.texCoord
        );

        this.shader.updateUniform(
            "u_MultiScatterColor",
            jsToGl(
                material.extensions?.KHR_materials_volume_scatter
                    ?.multiscatterColor
            )
        );
        ...
        for (; textureIndex < material.textures.length; ++textureIndex) {
            ...
            if (
                !this.webGl.setTexture(location, state.gltf, info, textureIndex)
            ) {
                continue;
            }
        }
        ...
        if (
            material.hasVolumeScatter &&
            sampledTextures?.scatterSampleTexture !== undefined
        ) {
            ...
            this.webGl.context.uniform1i(
                this.shader.getUniformLocation("u_ScatterFramebufferSampler"),
                textureCount
            );
            textureCount++;
            ...
            this.webGl.context.uniform1i(
                this.shader.getUniformLocation(
                    "u_ScatterDepthFramebufferSampler"
                ),
                textureCount
            );
            textureCount++;

            this.webGl.context.uniform1f(
                this.shader.getUniformLocation("u_MinRadius"),
                gltfMaterial.scatterMinRadius
            );
            this.webGl.context.uniform2i(
                this.shader.getUniformLocation("u_FramebufferSize"),
                renderpassConfiguration.frameBufferSize[0],
                renderpassConfiguration.frameBufferSize[1]
            );
            ...
            this.shader.updateUniformArray(
                "u_ScatterSamples",
        ...
        if (
            sampledTextures?.transmissionSampleTexture !== undefined &&
            state.environment &&
            state.renderingParameters.enabledExtensions
        ...
        if (drawIndexed) {
            ...
            if (instanceOffset !== undefined) {
                this.webGl.context.drawElementsInstanced(
                    primitive.mode,
                    indexAccessor.count,
                ...
                this.webGl.context.drawElements(
                    primitive.mode,
                    indexAccessor.count,
                    indexAccessor.componentType,
                    0
            ...
            if (instanceOffset !== undefined) {
                this.webGl.context.drawArraysInstanced(
                    primitive.mode,
                    0,
                    vertexCount,
                    instanceOffset.length
    ...
    getVisibleLights(gltf, nodes) {
        ...
        for (const nodeIndex of nodes) {
            ...
            if (node.children !== undefined) {
                nodeLights = nodeLights.concat(
                    this.getVisibleLights(gltf, node.children)
                );
            }
    ...
    pushFragParameterDefines(fragDefines, state) {
        ...
        let debugOutputMapping = [
            ...
            {
                debugOutput: GltfState.DebugOutput.generic.TANGENT,
                shaderDefine: "DEBUG_TANGENT"
            },
            {
            ...
            {
                debugOutput: GltfState.DebugOutput.generic.ALPHA,
                shaderDefine: "DEBUG_ALPHA"
            },
            {
                debugOutput: GltfState.DebugOutput.generic.UV_COORDS_0,
                shaderDefine: "DEBUG_UV_0"
            },
            {
                debugOutput: GltfState.DebugOutput.generic.UV_COORDS_1,
                shaderDefine: "DEBUG_UV_1"
            },
            {
            ...
            {
                debugOutput: GltfState.DebugOutput.generic.EMISSIVE,
                shaderDefine: "DEBUG_EMISSIVE"
            },

            {
                debugOutput: GltfState.DebugOutput.mr.BASECOLOR,
                shaderDefine: "DEBUG_BASE_COLOR"
            },
            {
                debugOutput: GltfState.DebugOutput.mr.ROUGHNESS,
                shaderDefine: "DEBUG_ROUGHNESS"
            },
            {
                debugOutput: GltfState.DebugOutput.mr.METALLIC,
                shaderDefine: "DEBUG_METALLIC"
            },
            ...
            {
                debugOutput:
                    GltfState.DebugOutput.clearcoat.CLEARCOAT_ROUGHNESS,
            ...
            {
                debugOutput:
                    GltfState.DebugOutput.transmission.TRANSMISSION_FACTOR,
                shaderDefine: "DEBUG_TRANSMISSION_FACTOR"
            },
            {
                debugOutput:
                    GltfState.DebugOutput.transmission.VOLUME_THICKNESS,
                shaderDefine: "DEBUG_VOLUME_THICKNESS"
            },

            {
                debugOutput:
                    GltfState.DebugOutput.diffuseTransmission
            ...
            {
                debugOutput:
                    GltfState.DebugOutput.iridescence.IRIDESCENCE_FACTOR,
                shaderDefine: "DEBUG_IRIDESCENCE_FACTOR"
            },
            {
                debugOutput:
                    GltfState.DebugOutput.iridescence.IRIDESCENCE_THICKNESS,
                shaderDefine: "DEBUG_IRIDESCENCE_THICKNESS"
            },

            {
                debugOutput:
                    GltfState.DebugOutput.anisotropy.ANISOTROPIC_STRENGTH,
                shaderDefine: "DEBUG_ANISOTROPIC_STRENGTH"
            },
            {
                debugOutput:
                    GltfState.DebugOutput.anisotropy.ANISOTROPIC_DIRECTION,
                shaderDefine: "DEBUG_ANISOTROPIC_DIRECTION"
            },

            {
                debugOutput:
                    GltfState.DebugOutput.volumeScatter.MULTI_SCATTER_COLOR,
                shaderDefine: "DEBUG_VOLUME_SCATTER_MULTI_SCATTER_COLOR"
            },
            {
                debugOutput:
                    GltfState.DebugOutput.volumeScatter.SINGLE_SCATTER_COLOR,
